...
image = (
    ...
    .run_commands(f"git clone --depth 1 https://github.com/VAST-AI-Research/TripoSR.git {TRIPOSR_DIR}")
    .env({"HF_HOME": f"{CACHE_DIR}/hf", "U2NET_HOME": f"{CACHE_DIR}/u2net", "PYTHONPATH": TRIPOSR_DIR})
)
...
class ImageTo3D:
    ...
    def generate(self, body: dict):
        ...
        t = time.time()
        response = requests.get(image_url, timeout=30, headers={"User-Agent": "webmcp-image-to-3d/1.0"})
        ...
        mesh = self.model.extract_mesh(scene_codes, True, resolution=MARCHING_CUBES_RESOLUTION)[0]
        ...
        timings["peak_gpu_gib"] = round(torch.cuda.max_memory_allocated() / 2**30, 2) if self.device == "cuda" else 0
        return {
