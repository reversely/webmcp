    ...
    <div className="wrap">
      <div>
        ...
        <section className="block" aria-labelledby="followups">
          <div className="labelrow"><h2 id="followups">Follow-ups</h2><span className="eyebrow">What replies still need</span></div>
          {follow_ups.length === 0 ? (
            <p className="hint" style={{ color: "var(--muted)" }} data-testid="followups-empty">Every reply is complete.</p>
          ) : (
        ...
        <section className="block" aria-labelledby="guests">
          ...
          {guests.length === 0 ? (
            <p className="hint" style={{ color: "var(--muted)" }} data-testid="guests-empty">No replies yet. Share the invite link to start.</p>
          ) : (
        ...
        <section className="block" aria-labelledby="gifts">
          <div className="labelrow"><h2 id="gifts">Guest Experience</h2><span className="eyebrow">Gifts and the order</span></div>
          <p className="hint" style={{ color: "var(--muted)" }} data-testid="gifts-empty">No gift chosen yet. Quantities follow the replies once one is.</p>
        </section>

        <section className="block" aria-labelledby="setup">
          <div className="labelrow"><h2 id="setup">Event setup</h2><span className="eyebrow">Edits go live at once</span></div>
          {editing ? <SetupForm snap={snap} onDone={() => { setEditing(false); onChanged(); }} /> : (
            <div className="list">
              <div className="row" style={{ gridTemplateColumns: "1fr auto" }}><span>{event.title}, hosted by {event.host || "no host named"}</span><button type="button" className="btn ghost small" onClick={() => setEditing(true)} data-testid="edit-setup">Edit</button></div>
              ...
              <div className="row" style={{ gridTemplateColumns: "1fr" }}><span className="type">{event.spots ? `${event.spots} spots` : "No spot limit"}; {event.cost_per_person_cents !== null ? `${money(event.cost_per_person_cents)} per person` : "no cost per person"}; {event.rsvp_deadline ? `reply by ${dateOnly(event.rsvp_deadline)}` : "no RSVP deadline"}</span></div>
            </div>
          )}
        </section>
      </div>
      ...
      <aside className="side">
        ...
        <div className="dark-card" data-testid="replies-card">
          <div className="in">
            <h2>{going} going</h2>
            <div className="when">Counts follow the replies as they arrive.</div>
...
function SetupForm({ snap, onDone }: { snap: Snapshot; onDone: () => void }) {
  const e = snap.event;
  const [form, setForm] = useState({ title: e.title, host: e.host, starts_at: e.starts_at.slice(0, 16), spots: e.spots?.toString() ?? "", cost: e.cost_per_person_cents !== null ? (e.cost_per_person_cents / 100).toString() : "", rsvp_deadline: e.rsvp_deadline ?? "", description: e.description, venue: e.venue });
  const [error, setError] = useState<string | null>(null);
  async function save() {
    const res = await fetch(`/api/events/${e.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: form.title, host: form.host, starts_at: new Date(form.starts_at).toISOString(), spots: form.spots ? Number(form.spots) : null, cost_per_person_cents: form.cost ? Math.round(Number(form.cost) * 100) : null, rsvp_deadline: form.rsvp_deadline || null, description: form.description, venue: form.venue }) });
