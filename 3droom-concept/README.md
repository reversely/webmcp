...
This app is one workspace of the [WebMCP sandbox](../README.md); the toolchain, hooks, and `.env` are set up at the repo root with `npm run setup`. Commands below run from this folder, or from the root with `-w 3droom-concept`.
...
```sh
cd ..; npm run setup; cd 3droom-concept   # once per clone, at the repo root
npm run dev                                # http://localhost:3000
```

## Environment variables

`.env` here is a link to the root `.env` (made by `npm run setup`); `.env.example` lists this app's names. The file is gitignored; never commit it.
...
The root `.pre-commit-config.yaml` runs ruff (lint and format on the Python files), whitespace and end-of-file fixes, JSON, YAML, and TOML checks, and `detect-secrets` against `.secrets.baseline`. The hook tools come from the uv dev group (`uv sync`), and `npm run setup` runs `uv run pre-commit install`. Run `uv run pre-commit run --files <paths>` before staging so a fixer never edits a file mid-commit.
